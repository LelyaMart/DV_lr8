...
with st.sidebar:
    ...
    if "sel_cat" not in st.session_state:
        st.session_state.sel_cat = all_categories.copy()

    with cat_col1:
        if st.button("✓ Все категории", use_container_width=True):
            st.session_state.sel_cat = all_categories.copy()
    ...
    if "sel_reg" not in st.session_state:
        st.session_state.sel_reg = all_regions.copy()

    with reg_col1:
        if st.button("✓ Все регионы", use_container_width=True):
            st.session_state.sel_reg = all_regions.copy()

    with reg_col2:
        if st.button("✕ Очистить регионы", use_container_width=True):
